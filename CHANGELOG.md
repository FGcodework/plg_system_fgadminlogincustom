...
## 1.16.4 (2026-08-03)
- Re-enabled `onContentPrepareForm` (Export/Import/Preset field auto-fill), following the language-string fix in 1.16.3. Behaviour matches v1.16.1 (type-preserving `$data->params` mutation, `try/catch` safety net), without the problematic literal script-tag text in a field description.

## 1.16.3 (2026-08-03)
- Removed a literal script-tag reference from the "Custom JavaScript" field's description text (`PLG_SYSTEM_FGADMINLOGINCUSTOM_CUSTOM_JS_DESC`). Joomla's admin rendering did not escape that sequence reliably, which corrupted the edit form's HTML (a missing hidden `task` input broke the Save / Save & Close / Close toolbar). Description rephrased without literal angle brackets.
- Checked all other language strings and XML descriptions/labels for the same pattern - none found.

## 1.16.2 (2026-08-03)
- Temporarily disabled the `onContentPrepareForm` handler (the Export/Import/Preset auto-fill logic) to restore the Save / Save & Close / Close toolbar, which had stopped working due to a corrupted admin form HTML structure. Export/Import/Preset did not apply anything in this version; all other features (logo, background, colors, shadows, custom CSS/JS on the login page) were unaffected.

## 1.16.1 (2026-08-03)
- Fixed a bug where an unrecognized shape of `$data->params` (a raw JSON string, which is how the `com_plugins` model returns it before Joomla calls `Form::bind()`) was replaced with an empty array, destroying the original data before the form could process it - this corrupted the admin edit screen (Save / Save & Close / Close stopped working, the Export/Import tab disappeared).
- `mergeIntoParams()` now preserves the original data type: a string stays a string (decoded, merged, re-encoded), a Registry/array/object is mutated in place as before, and a genuinely unrecognized type is left untouched rather than overwritten.
- `prepareAdminForm()` is wrapped in `try/catch` as a safety net.
...
- No functional change: the split preserves identical output to the previous monolithic implementation.
...
- The regex deliberately matches only a div carrying both the `main-brand` and `logo` classes (the slot above the form); the sidebar `#main-brand` without the `logo` class is left untouched. Fallback: if the slot does not exist, the logo is inserted as the first element of the `.login` box.
...
- FIX: the `hide_template_logo` and `hide_sidebar` selectors were too generic (`.view-login .logo`, `.main-brand`) and also hid the native login logo slot above the form. Narrowed down: the header logo only via `#header .logo`, the sidebar only via `#sidebar-wrapper`.
